...
begin
  ...
  while Varx > min_ do begin
      ErrorCal:=1000000;
      //ShowMessage(FloatToStr(f(Varx))+'   '+FloatToStr(f(Varx+vx)));
      biseccion(Varx,Varx+vx);
      Varx:=Varx-vx;
       //else begin ShowMessage('no hay interseccion'); exit; end
    end;
   ShowMessage('cnnnn'+IntToStr(Lr.Count)) ;
...
begin
  ...
  if (f(a)*f(b))>0 then begin
     ShowMessage('--'+FloatToStr(a)); ShowMessage('...'+FloatToStr(b));
     ShowMessage('noooooo');
     exit;     end
  else if (IsNan(f(a)) or IsNan(f(b))) then begin //exit;
     exit;     end
