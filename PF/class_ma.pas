...
begin
  try
   dp:=-4*power(x,3)+(exp(x)*(x-2))/power(x,3)+(9*power(x,2))-(power(2,x)/power(x,2))+(1/(power(x,2)))-((ln(x)/ln(10))/power(x,2))+(power(3,x)/(power(x-1,2)))-((power(3,x)*(ln(3)/ln(10)))/(x-1))+((power(2,x)*(ln(2)/ln(10)))/x);
   if dp=0 then
     begin
...
begin
      ...
      evaluador.AddVariable('x',x);
      //evaluador.AddVariable('y',y);
      evaluador.Expression:=dfunx;
      dp:=evaluador.Evaluate();
      if dp=0 then
         begin
              //showMessage('la derivacion da cero');
              dp:=dp+0.0000001;
